...
export default function ActiveCallWindow({ getWs }) {
  ...
  function toggleMute() {
    if (activeCall?.audio) {
      const m = activeCall.audio.toggleMute()
      setMuted(m)
    }
  }
  ...
  return (
    <div className={styles.window}>
      <div className={styles.header}>
        <Avatar name={activeCall.peerName} size={36} />
        <div className={styles.info}>
          <span className={styles.name}>{activeCall.peerName}</span>
          <span className={styles.status}>
            {activeCall.state === 'calling' ? '⏳ Дозваниваемся...' :
             activeCall.state === 'connected' ? `🟢 ${fmtElapsed()}` : activeCall.state}
          </span>
        </div>
        <div className={styles.controls}>
          <button className={`${styles.ctrl} ${muted ? styles.ctrlActive : ''}`} onClick={toggleMute} title="Мут">
            {muted
          ...
          <button className={styles.endBtn} onClick={endCall} title="Завершить">
